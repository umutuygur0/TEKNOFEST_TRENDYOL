"""
29_v23_calibrate.py — v23 Direct Threshold Calibration
=======================================================
Sorun: v23 quantile mapping, eğitim oranını (%17) test'e zorladı.
Çözüm: OOF'ta bulunan threshold'u (0.775) test'e DOĞRUDAN uygula.

Bu yaklaşım:
  ✓ Sabit oran hedeflemez (gizli bilgi)
  ✓ Modelin kendi öğrendiği eşiği kullanır
  ✓ OOF F1 maksimize eden threshold neyse, test'e aynısını uygular

v23 ile farklar:
  - BERT eğitimi YOK (bert_v23 cache'den)
  - BERT inference YOK (bert_scores_v23_*.npy cache'den)
  - Sadece: A (data) → C (features) → D (LGBM) → E (direct thr)
  - Tahmini süre: ~40 dk
  - test_proba raw olarak da kaydedilir (gelecekteki analizler için)
"""
...
# ── Stem + metrikler (v22/v23 ile aynı)
TR_SUFFIXES = sorted([
...
COLOR_FAMILY = {"antrasit":"gri","füme":"gri","platin":"gri","koyu gri":"gri","açık gri":"gri",
                "kurşun":"gri","lacivert":"mavi","indigo":"mavi","petrol":"mavi","bordo":"kırmızı",
                "altın":"sarı","gold":"sarı","gümüş":"metalik","krem":"bej","ekru":"bej"}
...
# ─────────────────────────────────────────────────────────────────────
# PHASE A: DATA + SAME NEGATIVES AS v23
# ─────────────────────────────────────────────────────────────────────
print("="*65, flush=True)
...
# Leak kontrolü
assert len(set(train_pairs["term_id"].unique()) & set(sub_pairs["term_id"].unique())) == 0, "LEAK!"
...
# ─────────────────────────────────────────────────────────────────────
# PHASE B: BERT SCORES (CACHE'DEN — v23 ile aynı dosyalar)
# ─────────────────────────────────────────────────────────────────────
print("\n[B] BERT SCORES (cache)...", flush=True)

v23_train_path = CACHE_DIR / "bert_scores_v23_train.npy"
v23_test_path  = SUBM / "bert_scores_v23_test.npy"

assert v23_train_path.exists(), "bert_scores_v23_train.npy bulunamadı! v23 tamamlandı mı?"
assert v23_test_path.exists(),  "bert_scores_v23_test.npy bulunamadı! v23 tamamlandı mı?"

train_bert = np.load(str(v23_train_path))
test_bert  = np.load(str(v23_test_path))

assert len(train_bert)==len(train_df), \
    f"BERT train boyutu uyuşmuyor: {len(train_bert)} vs {len(train_df)}"
assert len(test_bert)==len(sub_pairs), \
    f"BERT test boyutu uyuşmuyor: {len(test_bert)} vs {len(sub_pairs)}"

print(f"  Train BERT: {len(train_bert):,} ✓", flush=True)
print(f"  Test BERT : {len(test_bert):,} ✓", flush=True)

# ─────────────────────────────────────────────────────────────────────
# PHASE C: FEATURES (35 özellik, v23 ile aynı)
# ─────────────────────────────────────────────────────────────────────
...
X_tr, _ = build_features(train_df, tfidf, train_bert, fit=True,
...
# ─────────────────────────────────────────────────────────────────────
# PHASE D: LGBM 5-FOLD (v23 ile aynı)
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# PHASE E: SUBMISSION — DIRECT THRESHOLD (kalibrasyon)
# ─────────────────────────────────────────────────────────────────────
print("\n[E] TEST INFERENCE + DIRECT THRESHOLD...", flush=True)
X_te, _ = build_features(sub_pairs, tfidf, test_bert)
test_proba = sum(m.predict_proba(X_te[FEATS])[:,1] for m in models) / len(models)

# Raw proba kaydet (gelecekteki threshold denemeleri için)
proba_path = SUBM / "v23_test_proba.npy"
np.save(str(proba_path), test_proba)
print(f"  Raw test proba kaydedildi: {proba_path.name}", flush=True)

# KALİBRASYON: OOF threshold'unu doğrudan test'e uygula
# Sabit oran hedeflenmiyor — model ne öğrendiyse o kadar pozitif çıkar
final = (test_proba > best_thr).astype(int)
pos_n = final.sum()
print(f"\n  Direct thr={best_thr:.4f} → Pozitif={pos_n:,} ({100*pos_n/len(sub_pairs):.1f}%)", flush=True)

out_path = SUBM / "submission_v23_calibrated.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)

# Karşılaştırma için: eski quantile versiyonu da göster
oof_q     = np.mean(oof < best_thr)
q_thr     = np.quantile(test_proba, oof_q)
q_pos     = int((test_proba > q_thr).sum())
print(f"  Quantile thr={q_thr:.4f} → Pozitif={q_pos:,} ({100*q_pos/len(sub_pairs):.1f}%) [referans: eski yöntem]", flush=True)

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v23 Calibrated (Direct Threshold)", flush=True)
