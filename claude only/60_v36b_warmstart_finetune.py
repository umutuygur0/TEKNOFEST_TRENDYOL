"""
60_v36b_warmstart_finetune.py — v36b: ELECTRA'yı LLM-doğrulamalı zor örneklerle warm-start fine-tune et
====================================================================================================
Hedef: 0.881 (v36a, doğrulanmış) -> mümkün olduğunca yükseğe (liderlik tablosu 0.95-0.97'de).

v35a: gerçek 0.880 (+0.002). v36a: gerçek 0.881 (+0.001) — OOF F1 hafif düştü (0.96973)
ama gerçek skor yine de arttı. Naif proba-ensemble (v34+v35a+v36a ortalaması) denendi,
SONUÇ KÖTÜLEŞTİ (0.879) — proba dağılımları arasında basit ortalama sinyali sulandırıyor,
bu yöntem TERK EDİLDİ.

KÖK SORUN: v35a/v36a'da LLM-doğrulamalı yeni satırlar sadece LGBM'in feature girdisi olarak
eklendi (cache'li, DONMUŞ bert_v34 modeliyle skorlandı) — modelin kendisi bu yeni zor
örnekleri HİÇ görmedi, sadece LGBM üstündeki sinyali gördü. bert_score 3 versiyondur
importance tablosunda #1-2 sırada (%12) — yani darboğaz LGBM'in feature seti değil,
ELECTRA cross-encoder'ın kendisinin bu ayrımları anlayıp anlamaması.

v36b'NİN FİKRİ: ELECTRA'yı (bert_v34) SIFIRDAN DEĞİL, warm-start ile devam ettir — TÜM
LLM-doğrulamalı örnekleri (v35 train-side ~12K + v36 test-side ~13.75K, toplam ~25.8K,
TAM dahil) + nadir conflict tiplerini (gender/age/color) + rastgele tamamlamayla ~500-600K
örneklemle 2-3 epoch ek ince ayar yap (5 epoch/sıfırdan DEĞİL, düşük LR=1e-5). Bu, modelin
Türkçe sorgu-ürün alaka ayrımını gerçek test-zorluk seviyesinde YENİDEN öğrenmesini sağlıyor
— feature-seviyesi değil, model-seviyesi bir iyileştirme. Yeni model AYRI kaydediliyor:
`models/bert_v36/` (bert_v34 DOKUNULMUYOR, geri dönüş garantisi).

Model değiştiği için BERT skor cache'i GEÇERSİZ — train_df (~1.53M) ve test (3.36M) için
TAM inference gerekiyor (bu sürenin büyük kısmı, ~birkaç saat).

UYUMLULUK: Test sorgularından gelen LLM etiketleri SADECE eğitim sinyali (distillation) —
test satırları için LLM kararı ASLA doğrudan final tahmine yazılmıyor. Final tahmin yine
bu (yeniden eğitilmiş) ELECTRA+LGBM modelinden geliyor.

KESİN KAÇINILACAKLAR (yedi kez doğrulandı, değişmedi):
...
# ─────────────────────────────────────────────────────────────────────
# PHASE A2: v36a — LLM-DOĞRULAMALI SATIRLARI EKLE (train-side v35 + test-side v36)
# ─────────────────────────────────────────────────────────────────────
print("\n[A2] v36a: LLM-doğrulamalı satırlar ekleniyor (v35 train-side + v36 test-side)...", flush=True)

original_train_df = train_df.copy()   # BERT cache hizalaması için orijinal sırayı koru
...
existing_keys = set(original_train_df["term_id"] + "\t" + original_train_df["item_id"])
llm_parts = []
for llm_labels_path in LLM_LABEL_SOURCES:
    ...
    existing_keys |= set(part["term_id"] + "\t" + part["item_id"])   # dalgalar arası çakışmayı da önle

if llm_parts:
    llm_df = pd.concat(llm_parts, ignore_index=True)
    n_llm_pos = int((llm_df["label"] == 1).sum())
    n_llm_neg = int((llm_df["label"] == 0).sum())
    print(f"  Toplam LLM etiketli yeni satır: {len(llm_df):,}", flush=True)
    ...
    print(f"  Train (v36a genişletilmiş): {len(train_df):,} (orijinal {n_original:,} + LLM {len(llm_df):,})", flush=True)
else:
    print(f"  ✗ Hiçbir LLM etiket dosyası bulunamadı — v34 ile birebir aynı devam ediliyor", flush=True)
    llm_df = pd.DataFrame(columns=["term_id", "item_id", "label", "src"])

# ─────────────────────────────────────────────────────────────────────
# PHASE B: BERT — bert_v34'ten WARM-START, LLM-doğrulamalı zor örneklerle ince ayar
# ─────────────────────────────────────────────────────────────────────
print("\n[B] BERT v36 (bert_v34'ten warm-start, LLM-doğrulamalı stratified sample)...", flush=True)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"  Device: {device}", flush=True)

MODELS_DIR.mkdir(parents=True, exist_ok=True)
BERT_V36 = MODELS_DIR / "bert_v36"          # YENİ: bert_v34'e DOKUNULMUYOR
BERT_V36.mkdir(parents=True, exist_ok=True)
v36_train_bert_path = CACHE_DIR / "bert_scores_v36_train.npy"
v36_test_bert_path  = SUBM / "bert_scores_v36_test.npy"

# v36b: model her zaman yeniden ince ayar yapılır (warm-start) — SADECE bert_v36 klasörü
# zaten doluysa (bu script'in önceki bir çalıştırması tamamlanmışsa) atlanır.
bert_needs_finetune = not (BERT_V36 / "config.json").exists()
bert_needs_train    = (not v36_train_bert_path.exists() or
                       len(np.load(str(v36_train_bert_path))) != len(train_df))
bert_needs_test     = not v36_test_bert_path.exists()
...
if bert_needs_finetune:
    print("  BERT v36 warm-start fine-tune başlıyor (bert_v34'ten devam)...", flush=True)
    tokenizer  = AutoTokenizer.from_pretrained(str(BERT_V34))
    bert_model = AutoModelForSequenceClassification.from_pretrained(str(BERT_V34)).to(device)

    # ── v36b: TÜM LLM-doğrulamalı satırlar TAM dahil + nadir conflict tipleri TAM dahil,
    # kalanı rastgele doldur. Amaç: modelin gerçek test-zorluk seviyesindeki ayrımları
    # (v35 train-side + v36 test-side, ~25.8K) hiç kaçırmadan görmesi.
    RARE_CONFLICT_SRC = {"gender_conflict", "age_conflict", "color_conflict"}
    LLM_SRC = {"llm_recovered_positive", "llm_verified_negative",
               "llm_recovered_positive_test", "llm_verified_negative_test"}
    priority_mask = train_df["src"].isin(RARE_CONFLICT_SRC | LLM_SRC)
    ...
    print(f"  BERT_SAMPLE stratified: {len(bert_train):,} "
          f"(rare-conflict+LLM tam dahil: {n_priority:,}, rastgele tamamlama: {len(bert_train)-n_priority:,})", flush=True)
    ...
    # v36b: warm-start ince ayar — az epoch, düşük LR (5 epoch/2e-5 sıfırdan eğitim DEĞİL)
    ...
    bert_model.save_pretrained(str(BERT_V36))
    tokenizer.save_pretrained(str(BERT_V36))
    print(f"  BERT v36 saved → {BERT_V36}", flush=True)

else:
    print(f"  BERT v36 cache'de var → yükleniyor (yeniden eğitim YOK)", flush=True)
...
# v36b: model değişti (warm-start fine-tune) — TÜM train_df için TAM inference gerekiyor,
# eski cache (bert_scores_v34_*) artık bu modele karşılık gelmiyor.
if bert_needs_train:
    print(f"  Train BERT inference ({len(train_df):,} çift, YENİ v36 modeliyle)...", flush=True)
    t1 = time.time()
    ...
    np.save(str(v36_train_bert_path), train_bert)
    print(f"  → {v36_train_bert_path.name} | {(time.time()-t1)/60:.1f} dk", flush=True)
else:
    train_bert = np.load(str(v36_train_bert_path))
    print(f"  Train BERT cache'den: {len(train_bert):,}", flush=True)

if bert_needs_test:
    print(f"  Test BERT inference ({len(sub_pairs):,} çift, YENİ v36 modeliyle)...", flush=True)
    t1 = time.time()
    test_bert = bert_inference(sub_pairs["term_id"].tolist(), sub_pairs["item_id"].tolist(),
                               bert_model, tokenizer, desc="test")
    np.save(str(v36_test_bert_path), test_bert)
    print(f"  → {v36_test_bert_path.name} | {(time.time()-t1)/60:.1f} dk", flush=True)
else:
    test_bert = np.load(str(v36_test_bert_path))
...
out_path = SUBM / "submission_v36b_warmstart.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)
np.save(str(SUBM / "v36b_test_proba.npy"), test_proba.astype(np.float32))

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v36b ELECTRA warm-start fine-tune", flush=True)
print(f"  OOF F1        : {best_f1:.5f} (v34: {V34_OOF}, v35a: 0.96996, v36a: 0.96973)", flush=True)
print(f"  OOF threshold : {best_thr:.4f}", flush=True)
print(f"  Pozitif       : {pos_n:,} ({100*pos_n/len(sub_pairs):.1f}%)", flush=True)
print(f"  Train çifti   : {len(train_df):,} (orijinal {n_original:,} + LLM {len(llm_df):,})", flush=True)
print(f"  Feature sayısı: {len(FEATS)} (v34 ile aynı 41)", flush=True)
print(f"  BERT modeli   : bert_v36 (bert_v34'ten warm-start, 3 epoch, lr=1e-5)", flush=True)
