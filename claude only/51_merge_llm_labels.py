"""
51_merge_llm_labels.py — v35 Aşama 1: LLM doğrulama sonuçlarını birleştir
============================================================================
Tüm batch_*_result.csv dosyalarını (Claude'un doğruladığı pair_id -> verdict)
orijinal candidates.csv (term_id, item_id, query, title, ...) ile birleştirir.

Çıktı: claude only/51_llm_labels/merged_llm_labels.csv
  Kolonlar: term_id, item_id, label, src
    - verdict=1 → label=1, src="llm_recovered_positive"
    - verdict=0 → label=0, src="llm_verified_negative"

Bu dosya v35a script'inde train_df'e eklenecek yeni satırları oluşturur.
"""
...
LABELS_DIR = BASE / "claude only" / "51_llm_labels"
...
for f in result_files:
    try:
        try:
            ...
            # Bazı agent'lar "reason" alanında tırnaksız virgül bırakmış olabilir —
            # bozuk satırları atlayıp geri kalanı kurtar (python engine daha toleranslı)
...
print(f"\n  llm_recovered_positive (verdict=1): {n_pos:,} ({100*n_pos/len(merged):.1f}%)")
print(f"  llm_verified_negative  (verdict=0): {n_neg:,} ({100*n_neg/len(merged):.1f}%)")

out = pd.DataFrame({
    "term_id": merged["term_id"],
    "item_id": merged["item_id"],
    "label": merged["verdict"],
    "src": merged["verdict"].map({1: "llm_recovered_positive", 0: "llm_verified_negative"}),
})

out_path = LABELS_DIR / "merged_llm_labels.csv"
