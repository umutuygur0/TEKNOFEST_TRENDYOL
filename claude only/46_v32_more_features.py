"""
46_v32_more_features.py — v32: v31 + age_cross + material_mismatch + query_type + BERT test-cache fix
=======================================================================================================
Hedef: 0.86 (v31, doğrulanmış) -> 0.88

Temel: 45_v31_tyembed_parser.py ile BİREBİR AYNI
  - Aynı negatif üretimi (NEG_PER_POS=5, seed=42) — DEĞİŞMEDİ
  - Aynı BERT modeli mantığı (bert-base-turkish-cased, 5 epoch, v23 negatifleriyle)
  - v31'in 38 feature'ı (35 v23 + tyembed_cos + trusted_brand_mismatch + category_phrase_match)
  - Aynı GroupKFold(term_id) 5-fold binary LGBM, aynı direct-threshold kalibrasyonu

DÜZELTME: BERT train/test tutarsızlığı
  v31 çalışırken keşfedildi: bert_scores_v23_train.npy bu makinede taze eğitilen
  BERT ile üretildi (2026-07-05), ama submissions/bert_scores_v23_test.npy hâlâ
  ESKİ Asus makinesinden git'e commit edilmiş dosyaydı (2026-07-04, hiç değişmemiş).
  Yani train ve test BERT skorları FARKLI modellerden geliyordu (aynı tarif/seed
  ama farklı donanımda eğitilmiş, GPU non-determinism nedeniyle ağırlıklar birebir
  aynı değil). v31 script'indeki `bert_needs_test = not v23_test_bert_path.exists()`
  sadece dosya varlığına bakıyordu, modele değil.

  v32 düzeltmesi: models/bert_v23 klasörünün mtime'ı test-cache'ten daha yeniyse
  (ki şu an öyle — bu oturumda eğitildi) test BERT skorları BU modelle yeniden
  hesaplanır. Train BERT cache'i zaten bu oturumun modeliyle üretildiği için
  DOKUNULMUYOR (yeniden eğitim YOK, yeniden train-inference YOK — sadece test
  inference tekrarlanıyor, ~2.25 saat tahmini).

KASITLI OLARAK YAPILMAYANLAR (değişmedi — proje belleğinde kayıtlı 3 gerçek hata):
  ✗ Query-grubu-göreli özellik (v25/v26: Kaggle 0.84->0.68 çöküşü)
  ✗ Embedding/TF-IDF benzerliğiyle hard-negative mining (v24, v28/v29: unlabeled positive kirliliği)
  ✗ BERT'i riskli/yeni negatiflerle yeniden eğitmek (v30, yarım kalan deneme)

EKLENEN 3 YENİ ÖZELLİK (hepsi satır-bağımsız, query-grubu-göreli DEĞİL):
  1. age_cross     — gender_cross'un yaş versiyonu: query'de bebek/çocuk/yetişkin
                     kelimesi vs item age_group çatışması/uyumu (+1/-1/0).
                     Şimdiye kadar sadece age_in_q ("var mı") vardı, çatışma
                     sinyali hiç eklenmemişti (query_intent_analysis.md'de önerilmiş).
  2. material_mismatch — mevcut material_match (binary, sadece eşleşme) yanına
                     ternary sinyal ekler: query'de malzeme var + item'ın malzeme
                     attribute'ü FARKLIysa -1, eşleşiyorsa +1, yoksa 0
                     (trusted_brand_mismatch/color_typed ile aynı desen).
  3. query_type     — kategorik özellik (ana_kategori gibi pd.Categorical):
                     v31'de zaten hesaplanan query_trusted_brand()/query_category_phrase()
                     fonksiyonlarını kullanarak query'yi sınıflandırır:
                     brand_only / brand_category / brand_other / category_only /
                     category_attr / other. Hiç denenmemişti, ek maliyet ~sıfır.

DEĞİŞMEYENLER: LGBM ayarları (n_estimators=4000, early_stop=150) — v31'de foldlar
  2326-2862 iter'da erken durdu, kapasite sorunu yok. Threshold yöntemi (direct,
  quantile mapping yok) — v23 ve v31'de iki kez doğrulandı.

SAĞLIK KONTROLÜ (script sonunda otomatik basılır):
  - OOF F1 v31'in 0.9705'ine yakın olmalı.
  - Hiçbir yeni feature tek başına baskın olmamalı.
  - neg_src dağılımı v23/v31 ile birebir aynı olmalı.
"""
...
# ─────────────────────────────────────────────────────────────────────
# YENİ v32: age_cross (gender_cross'un yaş versiyonu)
# ─────────────────────────────────────────────────────────────────────
def age_cross(q, age_group):
    """+1 = query yaş kelimesi item age_group ile uyumlu
       -1 = query yaş kelimesi item age_group ile çatışıyor (örn. bebek query -> yetişkin item)
        0 = query'de yaş kelimesi yok veya item age_group unknown"""
...
# ─────────────────────────────────────────────────────────────────────
# YENİ v32: material_mismatch (material_match'in ternary versiyonu)
# ─────────────────────────────────────────────────────────────────────
def material_mismatch(q, attr_mat):
    """+1 = query'deki malzeme item'ın malzeme attribute'ü ile eşleşiyor
       -1 = query'de malzeme var, item'ın malzeme attribute'ü var ama FARKLI
        0 = query'de malzeme yok veya item'da malzeme attribute'ü yok"""
    q_mat = None
    for tok in q.split():
...
# ─────────────────────────────────────────────────────────────────────
# PHASE A: VERİ YÜKLEMESİ + ENHANCED NEGATİFLER (v23/v31 ile birebir aynı)
# ─────────────────────────────────────────────────────────────────────
print("=" * 65, flush=True)
...
print("\n[A2] Enhanced negatives (NEG_PER_POS=5, v23/v31 ile birebir aynı)...", flush=True)
...
print(f"  (v23/v31 ile karşılaştır — birebir aynı olmalı: kesin fark yoksa negatif üretimi bozulmamış demektir)", flush=True)
...
# DÜZELTME: test cache'i, mevcut modelden daha eskiyse yeniden hesaplanır
# ─────────────────────────────────────────────────────────────────────
...
bert_config_path    = BERT_V23 / "config.json"

bert_needs_finetune = not bert_config_path.exists()
bert_needs_train    = (not v23_train_bert_path.exists() or
                       len(np.load(str(v23_train_bert_path))) != len(train_df))

# v32 DÜZELTMESİ: sadece dosya varlığı değil, model test-cache'ten daha yeni mi?
if not v23_test_bert_path.exists():
    bert_needs_test = True
    test_cache_reason = "cache yok"
elif bert_config_path.exists() and bert_config_path.stat().st_mtime > v23_test_bert_path.stat().st_mtime:
    bert_needs_test = True
    test_cache_reason = "model test-cache'ten daha yeni (v31'de keşfedilen tutarsızlık düzeltiliyor)"
else:
    bert_needs_test = False
    test_cache_reason = ""

if bert_needs_test:
    print(f"  Test BERT yeniden hesaplanacak: {test_cache_reason}", flush=True)
...
if bert_needs_finetune:
    ...
    print(f"  BERT v23 cache'de var → yükleniyor (yeniden eğitim YOK)", flush=True)
...
if bert_needs_train:
    ...
    print(f"  Train BERT cache'den (bu oturumun modeliyle tutarlı): {len(train_bert):,}", flush=True)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE C: TY-ECOMM-EMBED COSINE (ham, grup feature YOK — v27/v31 yöntemi)
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
# YENİ v32: query_type kategorik özelliği
# ─────────────────────────────────────────────────────────────────────
def query_type(q):
    qb = query_trusted_brand(q)
...
print(f"  Örnek query_type: {[(q, query_type(q)) for q in list(terms['query'].head(5))]}", flush=True)

# ─────────────────────────────────────────────────────────────────────
# PHASE D: FEATURE ENGINEERING (38 v31 + 3 yeni = 41 özellik)
# ─────────────────────────────────────────────────────────────────────
...
def build_features(df_in, tfidf_vect, bert_arr, tyembed_arr, fit=False, all_texts=None):
    ...
    # ── YENİ v32 ÖZELLİKLERİ (satır bağımsız, query-grubu-göreli DEĞİL)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE E: BINARY LGBMClassifier 5-FOLD (v31 ile aynı ayarlar)
# ─────────────────────────────────────────────────────────────────────
...
print("=" * 65, flush=True)
V31_OOF = 0.9705
if best_f1 > V31_OOF + 0.02:
    print(f"  ⚠ UYARI: OOF F1 ({best_f1:.4f}) v31'in {V31_OOF}'ından çok yüksek!", flush=True)
    ...
    print(f"  ✓ OOF F1 ({best_f1:.4f}) v31'e ({V31_OOF}) yakın/altında — makul bir sinyal.", flush=True)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE F: TEST INFERENCE + SUBMISSION (DOĞRUDAN threshold — v23/v31 yöntemi)
# ─────────────────────────────────────────────────────────────────────
...
out_path = SUBM / "submission_v32_more_features.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)
np.save(str(SUBM / "v32_test_proba.npy"), test_proba.astype(np.float32))

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v32 more features (age_cross, material_mismatch, query_type)", flush=True)
...
print(f"  Feature sayısı: {len(FEATS)} (38 v31 + 3 yeni)", flush=True)
