"""
49_v34_new_encoder.py — v34: cross-encoder'ı değiştir (ELECTRA), geri kalan HER ŞEY v33 ile aynı
====================================================================================================
Hedef: 0.872 (v32=v33, doğrulanmış) -> 0.88 (mümkünse 0.885)

v31 (+0.028 kazanç) sonrası v32 (+0.004) ve v33 (0, düz) üst üste azalan getiri gösterdi.
Feature importance 3 versiyondur hep aynı iki sinyalde tıkanıyor: bert_score (%13) ve
tyembed_cos. Yani darboğaz artık feature mühendisliği değil, BERT modelinin kendisi.

v34'ün TEK değişikliği: cross-encoder modeli `dbmdz/bert-base-turkish-cased` yerine
`dbmdz/electra-base-turkish-cased-discriminator`. Seçim süreci:
  1. Önce boun-tabilab/TabiBERT (ModernBERT mimarili, Türkçe, Aralık 2025) denendi —
     duman testinde (48_tabibert_smoketest.py) sağlıklı çalıştı (loss düzgün düştü, hata yok)
     AMA inference hızı mevcut modelden %38 YAVAŞ çıktı (kısa sekans görevimizde ModernBERT'in
     hız avantajı ortaya çıkmadı) — tam koşu ~11.2 saat tahmin edildi, mevcut modelden daha kötü.
  2. dbmdz/electra-base-turkish-cased-discriminator'a geçildi: hızlı uyumluluk kontrolünde
     (log_electra_check.txt) sağlıklı yüklendi (110.6M parametre), loss düzgün düştü,
     ve küçük ölçekte ÇOK daha hızlı çıktı (~5300 satır/sn vs TabiBERT ~292, dbmdz ~471).
     ELECTRA'nın discriminator ön-eğitim hedefi literatürde aynı boyuttaki BERT'ten
     fine-tune'da genelde daha güçlü çıkıyor — hem hız hem potansiyel kalite avantajı.

Yeni model AYRI kaydediliyor: `models/bert_v34/` — `models/bert_v23` ve `models/bert_v33`
DOKUNULMUYOR (önceki doğrulanmış modeller korunuyor, geri dönüş garantisi).

KESİN KAÇINILACAKLAR (dört kez doğrulandı, değişmedi — model değişse de geçerli):
  ✗ Query-grubu-göreli özellik (v25/v26: 0.84->0.68 çöküşü)
  ✗ Embedding/TF-IDF benzerliğiyle hard-negative mining (v24, v28/v29)
  ✗ BERT'i similarity-mined negatiflerle eğitmek (v30)

DEĞİŞMEYENLER: v33'ün TAMAMI — color_conflict dahil 7 negatif kaynağı, stratified BERT
  sample mantığı, 41 feature'ın tamamı, LGBM ayarları (n_estimators=4000, early_stop=150),
  threshold yöntemi (direct, quantile mapping yok), NEG_PER_POS=5, seed=42.

SAĞLIK KONTROLÜ (script sonunda otomatik basılır):
  - Negatif kaynak dağılımı v23/v31/v32/v33 ile aynı olmalı (model değişikliği bunu etkilemez).
  - BERT_SAMPLE stratifikasyonu aynı şekilde çalışmalı.
  - OOF F1 v33'ün 0.96956'sına yakın olmalı.
  - Hiçbir feature tek başına baskın olmamalı.
  - bert_score'un feature importance'taki payı v33 ile kıyaslanacak.
"""
...
bert_needs_train    = (not v34_train_bert_path.exists() or
                       len(np.load(str(v34_train_bert_path))) != len(train_df))
...
if bert_needs_train:
    ...
    np.save(str(v34_train_bert_path), train_bert)
...
print("=" * 65, flush=True)
V33_OOF = 0.96956
if best_f1 > V33_OOF + 0.02:
    print(f"  ⚠ UYARI: OOF F1 ({best_f1:.4f}) v33'ün {V33_OOF}'ından çok yüksek!", flush=True)
    ...
    print(f"  ✓ OOF F1 ({best_f1:.4f}) v33'e ({V33_OOF}) yakın/altında — makul bir sinyal.", flush=True)
...
out_path = SUBM / "submission_v34_new_encoder.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)
np.save(str(SUBM / "v34_test_proba.npy"), test_proba.astype(np.float32))

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v34 new encoder (ELECTRA)", flush=True)
...
print(f"  Feature sayısı: {len(FEATS)} (v33 ile aynı 41, BERT modeli farklı: ELECTRA)", flush=True)
