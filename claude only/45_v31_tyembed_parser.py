"""
45_v31_tyembed_parser.py — v31: v23 + tyembed_cos (ham) + trusted_brand_mismatch + category_phrase_match
=============================================================================================================
Hedef: 0.84 (v23, doğrulanmış en iyi) -> 0.86

Temel: 28_v23_combined.py ile BİREBİR AYNI
  - Aynı negatif üretimi (NEG_PER_POS=5: same_brand_diff_main, same_brand_diff_sub,
    gender_conflict, age_conflict, same_main_category, different_main_category)
  - Aynı BERT fine-tune (bert-base-turkish-cased, 5 epoch, 500K sample, v23 negatifleriyle)
  - Aynı 35 v23 feature'ı
  - Aynı GroupKFold(term_id) 5-fold binary LGBM

KASITLI OLARAK YAPILMAYANLAR (geçmiş gerçek hatalar — proje belleğinde kayıtlı):
  ✗ Query-grubu-göreli özellik (percentile/z-score/rank-in-group)
    → v25/v26'da bu tür özellikler (bert_grp_pct vb.) OOF'u 0.96'ya çıkardı ama
      gerçek Kaggle skoru 0.84'ten 0.68'e ÇÖKTÜ. Sebep: train'de query başına ~6 aday
      (sentetik negatif), test'te ~100+ aday (Trendyol retrieval) var — "gruptaki
      yüzdelik dilim" iki ortamda tamamen farklı anlam taşıyor. BİR DAHA EKLENMEYECEK.
  ✗ Embedding/TF-IDF benzerliğiyle "hard negative" madenciliği
    → v24 (TF-IDF top-300) ve v28/v29 (FAISS NN top-50) İKİ KEZ aynı şekilde patladı:
      benzer ama etiketlenmemiş ürünler negatif diye işaretlenip gerçek pozitifleri
      kirletti (v24: OOF 0.048 düştü, Kaggle 0.84->0.79). BİR DAHA DENENMEYECEK.
  ✗ BERT'i riskli/yeni negatiflerle yeniden eğitmek (v30'un yarım kalan denemesi)
    → BERT SADECE v23'ün kendi güvenli negatifleriyle eğitiliyor, başka hiçbir şeyle değil.

EKLENEN 3 ŞEY (hepsi query-grubu-göreli DEĞİL, her satır bağımsız hesaplanıyor):
  1. tyembed_cos       — Trendyol'un kendi embedding modeli (TY-ecomm-embed), HAM cosine,
                         grup özelliği YOK (v27'nin düzeltmesiyle birebir aynı yöntem).
  2. trusted_brand_mismatch — query'de "güvenilir marka" (jenerik kelime olmayan, items.brand'de
                         yeterince sık geçen tek/çok kelimelik marka) varsa ve item brand'i
                         farklıysa -1, eşleşiyorsa +1, query'de marka yoksa 0.
                         (next_step_feasibility_research.md'de araştırılmış ama hiç pipeline'a
                         girmemişti: brand-match satırlarda %98.9 pozitif oranı vs
                         brand-mismatch satırlarda %23 — güçlü, kullanılmamış sinyal.)
  3. category_phrase_match  — query içindeki bilinen kategori ifadesi (items.category zincirinin
                         kendi segmentlerinden türetilir, örn. "spor ayakkabı", "şarj aleti")
                         adayın kategori zincirinde birebir geçiyor mu (+1/-1/0).
                         (query_intent_analysis.md'de araştırılmış ama pipeline'a hiç girmemişti.)

KÜÇÜK GÜVENLİ İYİLEŞTİRME: n_estimators 2000->4000, early_stopping 100->150
  (v23'te bazı foldlar iter=1998-2000 tavanına takılmıştı; v24'ün underfitting
   dersini tersine çevirip biraz headroom veriyoruz, davranış değişmiyor.)

THRESHOLD: 29_v23_calibrate.py'daki DOĞRUDAN yöntem — OOF F1'i maksimize eden
  threshold test'e AYNEN uygulanır (quantile mapping YOK).

SAĞLIK KONTROLÜ (script sonunda otomatik basılır):
  - OOF F1 v23'ün 0.9686'sından çok fazla sıçramamalı (0.97-0.975 iyi işaret,
    0.95+ ise v25/26/27 gibi kırmızı bayrak — feature importance'a bak).
  - Hiçbir yeni feature tek başına baskın olmamalı.
  - neg_src dağılımı v23 ile birebir aynı olmalı (yeni negatif stratejisi YOK).
"""
...
# Bu makineye göre otomatik (Asus'a hardcode bağımlılık yok)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE A: VERİ YÜKLEMESİ + ENHANCED NEGATİFLER (v23 ile birebir aynı)
# ─────────────────────────────────────────────────────────────────────
print("=" * 65, flush=True)
...
print("\n[A2] Enhanced negatives (NEG_PER_POS=5, v23 ile birebir aynı)...", flush=True)
...
print(f"  (v23 ile karşılaştır — birebir aynı olmalı: kesin fark yoksa negatif üretimi bozulmamış demektir)", flush=True)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE B: BERT (v23'ün kendi güvenli negatifleriyle — YENİ negatif YOK)
# ─────────────────────────────────────────────────────────────────────
print("\n[B] BERT (v23 negatifleriyle fine-tune veya cache)...", flush=True)
...
# PERFORMANS NOTU: Orijinal v23 scripti PairDataset.__getitem__ içinde HER
# örneği tek tek tokenize ediyordu (num_workers=0) — bu makinede bu CPU-bound
# darboğaz oldu (GPU %58 kullanımda takıldı, tahmini süre ~10 saate çıktı).
# Windows'ta num_workers>0 için script'i "if __name__ == '__main__':" ile
# sarmalamak gerekir (spawn metodu tüm modülü yeniden import eder) — bunun
# yerine daha basit ve güvenli bir çözüm: TÜM metinleri tokenizer'ın hızlı
# (Rust) toplu (batch) modunda ÖNCEDEN tensöre çeviriyoruz, sonra sadece
# tensör indexliyoruz. Model/negatif/feature mantığı DEĞİŞMEDİ, sadece
# tokenize etme şekli hızlandırıldı.
...
def bulk_tokenize(queries, items_t, tokenizer, max_len=128, chunk=20_000):
    """Fast tokenizer ile büyük parçalar halinde toplu tokenize (num_workers gerektirmez)."""
...
if bert_needs_finetune:
    print("  BERT fine-tune başlıyor (v23'ün güvenli negatifleriyle)...", flush=True)
    ...
    print(f"  → tokenize {BERT_SAMPLE:,} örnek | {time.time()-t_tok:.1f}s", flush=True)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE C: TY-ECOMM-EMBED COSINE (ham, grup feature YOK — v27'nin düzeltmesi)
# ─────────────────────────────────────────────────────────────────────
print("\n[C] TY-ECOMM-EMBED COSINE (grup feature olmadan, v27 yöntemi)...", flush=True)
t1 = time.time()
...
# ─────────────────────────────────────────────────────────────────────
# PHASE C2: TRUSTED BRAND + CATEGORY PHRASE SÖZLÜKLERİ (yeni v31)
# ─────────────────────────────────────────────────────────────────────
...
for b, cnt in _brand_counts.items():
    if " " in b:
        TRUSTED_BRANDS.add(b)          # çok kelimeli marka string'i doğal olarak daha güvenli
...
print(f"  Örnek trusted brand: {list(TRUSTED_BRANDS)[:10]}", flush=True)
...
def trusted_brand_signal(q, item_brand):
    """+1 = query'deki güvenilir marka item brand ile eşleşiyor
       -1 = query'de güvenilir marka var ama item brand farklı (güçlü negatif sinyal)
        0 = query'de güvenilir marka yok (feature pasif)"""
...
def category_phrase_signal(q, item_category):
    """+1 = query'deki bilinen kategori ifadesi adayın kategori zincirinde geçiyor
       -1 = query'de kategori ifadesi var ama adayın kategori zincirinde yok
        0 = query'de tanınan bir kategori ifadesi yok"""
    phrase = query_category_phrase(q)
    if phrase is None: return 0.0
    return 1.0 if phrase in item_category else -1.0

# ─────────────────────────────────────────────────────────────────────
# PHASE D: FEATURE ENGINEERING (35 v23 + 3 yeni = 38 özellik)
# ─────────────────────────────────────────────────────────────────────
...
def build_features(df_in, tfidf_vect, bert_arr, tyembed_arr, fit=False, all_texts=None):
    ...
    # ── YENİ v31 ÖZELLİKLERİ (query-grubu-göreli DEĞİL, satır bağımsız)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE E: BINARY LGBMClassifier 5-FOLD
# (n_estimators 2000->4000, early_stopping 100->150 — v24'ün underfitting
#  dersini tersine çeviriyoruz, davranış değişmiyor sadece headroom artıyor)
# ─────────────────────────────────────────────────────────────────────
...
print("=" * 65, flush=True)
V23_OOF = 0.9686
if best_f1 > V23_OOF + 0.02:
    print(f"  ⚠ UYARI: OOF F1 ({best_f1:.4f}) v23'ün {V23_OOF}'ından çok yüksek!", flush=True)
    ...
    print(f"  ✓ OOF F1 ({best_f1:.4f}) v23'e ({V23_OOF}) yakın/altında — makul bir sinyal.", flush=True)
...
# ─────────────────────────────────────────────────────────────────────
# PHASE F: TEST INFERENCE + SUBMISSION (DOĞRUDAN threshold — 29_v23_calibrate.py yöntemi)
# ─────────────────────────────────────────────────────────────────────
...
out_path = SUBM / "submission_v31_tyembed_parser.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)
np.save(str(SUBM / "v31_test_proba.npy"), test_proba.astype(np.float32))

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v31 tyembed + parser features", flush=True)
...
print(f"  Feature sayısı: {len(FEATS)} (35 v23 + 3 yeni)", flush=True)
