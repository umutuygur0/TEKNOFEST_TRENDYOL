"""
41_v28b_nn_iter16k.py — v28: FAISS NN Hard Negatives
=====================================================
v27 tabanı. Tek fark: negatif örnekleme stratejisi.

SORUN (v26 / v23 gap): diff_main_category (%40) çok kolay negatifler.
  Test'te Trendyol'un retrieval sistemi semantik benzer adaylar sunuyor.
  Model kolay negatiflerde çalışmayı öğreniyor → test'te şaşırıyor.

ÇÖZÜM: FAISS ile her sorgu için TY-ecomm-embed top-50 NN bul.
  Rank 1-10: atla (unlabeled positive riski yüksek)
  Rank 11-60: hard negative olarak kullan
  Kalan slotlar: mevcut strateji (same_brand, gender, age)
  diff_main_category: son çare (max 1 per query)

Hipotez: train/test neg dağılımı yaklaşır → OOF-Kaggle gap küçülür.
Beklenti: 0.85-0.87

Cache: 36_tyembed_cpuonly.py ile üretilmiş olmalı.
"""
...
for p in [ITEM_EMB, ITEM_IDS, TRAIN_Q_EMB, TRAIN_Q_IDS, TEST_Q_EMB, TEST_Q_IDS]:
    if not p.exists():
        print(f"EKSIK CACHE: {p}"); sys.exit(1)
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE A0: TY-ECOMM EMBEDDINGS + FAISS INDEX (negatif sampling için)
# ═══════════════════════════════════════════════════════════════════════
t0 = time.time()
print("=" * 65, flush=True)
print("[A0] TY-ECOMM EMBEDDINGS YUKLE + FAISS INDEX...", flush=True)

item_embs    = np.load(str(ITEM_EMB))
item_ids_raw = np.load(str(ITEM_IDS), allow_pickle=True)
train_q_embs_nn = np.load(str(TRAIN_Q_EMB))
train_q_ids_nn  = np.load(str(TRAIN_Q_IDS), allow_pickle=True)
test_q_embs  = np.load(str(TEST_Q_EMB))
test_q_ids   = np.load(str(TEST_Q_IDS), allow_pickle=True)

print(f"  item_embs: {item_embs.shape} | train_q: {train_q_embs_nn.shape}", flush=True)

dim = item_embs.shape[1]
faiss_index = faiss.IndexFlatIP(dim)
faiss_index.add(item_embs.astype(np.float32))
print(f"  FAISS IndexFlatIP: {faiss_index.ntotal:,} items, dim={dim}", flush=True)

TOP_SEARCH = 60
TOP_SKIP   = 10  # rank 1-10 atla (unlabeled positive riski)
print(f"  {len(train_q_ids_nn):,} train sorgu icin top-{TOP_SEARCH} NN arama...", flush=True)
t_faiss = time.time()
_, I_nn = faiss_index.search(train_q_embs_nn.astype(np.float32), TOP_SEARCH)
print(f"  FAISS arama tamamlandi: {time.time()-t_faiss:.1f}s", flush=True)

idx_to_iid    = {i: str(iid) for i, iid in enumerate(item_ids_raw)}
tid_to_nn_pos = {str(tid): i for i, tid in enumerate(train_q_ids_nn)}

# ═══════════════════════════════════════════════════════════════════════
# PHASE A: VERİ + NEGATİFLER (FAISS NN öncelikli)
# ═══════════════════════════════════════════════════════════════════════
print("\n[A] VERİ + NEGATİFLER (FAISS NN hard neg, rng=42)...", flush=True)
...
# Train/test leak kontrolu
train_tids = set(train_pairs["term_id"].unique())
test_tids  = set(sub_pairs["term_id"].unique())
assert len(train_tids & test_tids) == 0, "LEAK TESPIT EDILDI!"
print(f"  {len(items):,} urun | {len(sub_pairs):,} test cifti", flush=True)
print(f"  train/test term_id disjoint: {len(train_tids):,} train, {len(test_tids):,} test", flush=True)

# Sorgu başına pozitif set (FAISS NN filtreleme için)
query_positives = {}
for _, row in train_pairs.iterrows():
    tid = str(row["term_id"])
    iid = str(row["item_id"])
    if tid not in query_positives:
        query_positives[tid] = set()
    query_positives[tid].add(iid)

# NN hard neg pool oluştur
hard_neg_pool = {}
nn_pool_ptr   = {}
for i, tid in enumerate(train_q_ids_nn):
    tid_str  = str(tid)
    pos_ids  = query_positives.get(tid_str, set())
    nn_iids  = [idx_to_iid[idx] for idx in I_nn[i, TOP_SKIP:]]  # rank TOP_SKIP..TOP_SEARCH
    pool = [iid for iid in nn_iids if iid not in pos_ids]
    rng_build = np.random.default_rng(hash(tid_str) % (2**32))
    rng_build.shuffle(pool)
    hard_neg_pool[tid_str] = pool
    nn_pool_ptr[tid_str]   = 0

avg_pool = np.mean([len(v) for v in hard_neg_pool.values()])
print(f"  NN hard neg pool: {len(hard_neg_pool):,} sorgu, ortalama pool={avg_pool:.1f} item", flush=True)

positive_keys = set(train_pairs["term_id"].astype(str) + "\t" + train_pairs["item_id"].astype(str))
used_keys: set = set()
rng = np.random.default_rng(42)

def sample_nn_hard(term_id, pos_iid):
    pool = hard_neg_pool.get(term_id, [])
    ptr  = nn_pool_ptr.get(term_id, 0)
    while ptr < len(pool):
        iid = pool[ptr]; ptr += 1
        k   = term_id + "\t" + iid
        if iid != pos_iid and k not in positive_keys and k not in used_keys:
            nn_pool_ptr[term_id] = ptr
            return iid
    nn_pool_ptr[term_id] = ptr
    return None
...
nn_pct = 100 * src_counts.get("nn_hard_neg", 0) / sum(src_counts.values())
print(f"  NN hard neg orani: {nn_pct:.1f}%", flush=True)
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE C: TY-ECOMM-EMBED COSINE (embeddings zaten yüklü)
# ═══════════════════════════════════════════════════════════════════════
print("\n[C] TY-ECOMM-EMBED COSINE...", flush=True)
t1 = time.time()

iid_to_emb_idx       = {str(iid): i for i, iid in enumerate(item_ids_raw)}
train_tid_to_emb_idx = {str(tid): i for i, tid in enumerate(train_q_ids_nn)}
...
train_cos = compute_cosine(train_df, train_q_embs_nn, train_tid_to_emb_idx, item_embs, iid_to_emb_idx)
print(f"  train_cos: pos mean={train_cos[train_df['label'].values==1].mean():.3f} "
      f"neg={train_cos[train_df['label'].values==0].mean():.3f}", flush=True)

print("  Test cosine...", flush=True)
test_cos = compute_cosine(sub_pairs, test_q_embs, test_tid_to_emb_idx, item_embs, iid_to_emb_idx)
print(f"  test_cos: mean={test_cos.mean():.3f}", flush=True)
print(f"  → {(time.time()-t1)/60:.1f} dk", flush=True)

del item_embs, train_q_embs_nn, test_q_embs
gc.collect()
...
for fold, (tri, vli) in enumerate(gkf.split(X_tr, y, tids)):
    model = LGBMClassifier(
        n_estimators=16000, learning_rate=0.03, num_leaves=127, max_depth=8,
