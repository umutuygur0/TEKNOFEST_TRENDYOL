"""
52_v35a_llm_verified_quicktest.py — v35a: LLM-doğrulamalı veri ekleme (hızlı test, v34 tabanlı)
====================================================================================================
Hedef: 0.878 (v34, doğrulanmış) -> mümkün olduğunca yükseğe (liderlik tablosu 0.95-0.97'ye çıktı).

FİKİR: v24/v28'in yaptığı "embedding benzerliğiyle hard-negative mine et" yaklaşımı hep
kirlilik nedeniyle skoru düşürmüştü (~%29'u aslında etiketlenmemiş gerçek pozitifti).
v35'te aynı adayları ÜRETİYORUZ ama körlemesine negatif saymak yerine Claude'a (LLM) toplu
halde gösterip GERÇEKTEN doğruluyoruz (50_llm_candidate_gen.py + 51_merge_llm_labels.py).
Sonuç: claude only/51_llm_labels/merged_llm_labels.csv — ~90% llm_recovered_positive
(training_pairs.csv'nin seyrek etiketlemesini telafi ediyor), ~10% llm_verified_negative
(güvenli, doğrulanmış hard negative — modelin hiç görmediği zorluk seviyesi).

v35a'nın DEĞİŞİKLİĞİ: Phase A (negatif üretimi) v34 ile BİREBİR AYNI kalıyor (aynı seed=42,
aynı 7 kaynak) — bu orijinal train_df'i oluşturuyor. Sonra merged_llm_labels.csv'deki yeni
satırlar (mevcutlarla çakışmayanlar) train_df'e EKLENİYOR. BERT skorları için: orijinal
satırlar cache'den (bert_scores_v34_train.npy) okunuyor, SADECE yeni LLM satırları için
...
KESİN KAÇINILACAKLAR (beş kez doğrulandı, değişmedi):
  ...
  ✗ Final inference adımında paid/LLM model kullanmak (yasak) — LLM SADECE eğitim
    verisi üretiminde kullanıldı, nihai tahmin hâlâ ELECTRA+LGBM.
...
SAĞLIK KONTROLÜ (script sonunda otomatik basılır):
  ...
  - LLM kaynak dağılımı (llm_recovered_positive / llm_verified_negative) makul olmalı.
"""
...
# ─────────────────────────────────────────────────────────────────────
# PHASE A2: v35a — LLM-DOĞRULAMALI SATIRLARI EKLE
# ─────────────────────────────────────────────────────────────────────
print("\n[A2] v35a: LLM-doğrulamalı satırlar ekleniyor...", flush=True)

original_train_df = train_df.copy()   # BERT cache hizalaması için orijinal sırayı koru
n_original = len(original_train_df)

llm_labels_path = BASE / "claude only" / "51_llm_labels" / "merged_llm_labels.csv"
if llm_labels_path.exists():
    ...
    llm_df = llm_labels[["term_id", "item_id", "label", "src"]].copy()
    llm_df["label"] = llm_df["label"].astype(int)

    n_llm_pos = int((llm_df["label"] == 1).sum())
    n_llm_neg = int((llm_df["label"] == 0).sum())
    print(f"  LLM etiketli yeni satır: {len(llm_df):,} (mevcutlarla çakışan/duplike elendi)", flush=True)
    print(f"    llm_recovered_positive: {n_llm_pos:,} | llm_verified_negative: {n_llm_neg:,}", flush=True)

    train_df = pd.concat([original_train_df, llm_df], ignore_index=True)
    print(f"  Train (v35a genişletilmiş): {len(train_df):,} (orijinal {n_original:,} + LLM {len(llm_df):,})", flush=True)
else:
    print(f"  ✗ {llm_labels_path} bulunamadı — LLM verisi olmadan v34 ile birebir aynı devam ediliyor", flush=True)
    llm_df = pd.DataFrame(columns=["term_id", "item_id", "label", "src"])

# ─────────────────────────────────────────────────────────────────────
# PHASE B: BERT — v34 MODELİ YENİDEN KULLANILIYOR (yeniden eğitim YOK),
#          sadece YENİ LLM satırları için ek inference
# ─────────────────────────────────────────────────────────────────────
print("\n[B] BERT v34 (cache + sadece yeni LLM satırları için inference)...", flush=True)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"  Device: {device}", flush=True)

MODELS_DIR.mkdir(parents=True, exist_ok=True)
BERT_V34.mkdir(parents=True, exist_ok=True)
v34_train_bert_path = CACHE_DIR / "bert_scores_v34_train.npy"
v34_test_bert_path  = SUBM / "bert_scores_v34_test.npy"

bert_needs_finetune = not (BERT_V34 / "config.json").exists()
# v35a: cache her zaman ORİJİNAL (v34 ile birebir aynı) satır sayısına göre doğrulanır —
# LLM ile genişletilmiş train_df'in tam uzunluğuna göre DEĞİL (cache sadece orijinali kapsar,
# LLM satırları için ayrıca ve sadece bir kez inference yapılacak).
orig_cache_valid = (v34_train_bert_path.exists() and
                    len(np.load(str(v34_train_bert_path))) == n_original)
bert_needs_test     = not v34_test_bert_path.exists()
...
if bert_needs_finetune:
    ...
    rare_mask = original_train_df["src"].isin(RARE_CONFLICT_SRC)
    rare_df   = original_train_df[rare_mask]
    rest_df   = original_train_df[~rare_mask]

    BERT_SAMPLE = min(500_000, len(original_train_df))
    n_rare = len(rare_df)
    ...
    bert_model.save_pretrained(str(BERT_V34))
    tokenizer.save_pretrained(str(BERT_V34))
    print(f"  BERT v34 saved → {BERT_V34}", flush=True)

else:
    print(f"  BERT v34 cache'de var → yükleniyor (yeniden eğitim YOK)", flush=True)
...
if orig_cache_valid:
    train_bert_orig = np.load(str(v34_train_bert_path))
    print(f"  Orijinal train BERT cache'den: {len(train_bert_orig):,}", flush=True)
else:
    print(f"  Orijinal train BERT inference ({n_original:,} çift)...", flush=True)
    t1 = time.time()
    train_bert_orig = bert_inference(
        original_train_df["term_id"].astype(str).tolist(),
        original_train_df["item_id"].astype(str).tolist(),
        bert_model, tokenizer, desc="train-orig")
    np.save(str(v34_train_bert_path), train_bert_orig)
    print(f"  → {v34_train_bert_path.name} | {(time.time()-t1)/60:.1f} dk", flush=True)

if len(llm_df) > 0:
    print(f"  LLM-doğrulamalı YENİ satırlar için BERT inference ({len(llm_df):,} çift, "
          f"cache'li ELECTRA modeliyle, yeniden eğitim YOK)...", flush=True)
    t1 = time.time()
    train_bert_llm = bert_inference(
        llm_df["term_id"].astype(str).tolist(),
        llm_df["item_id"].astype(str).tolist(),
        bert_model, tokenizer, desc="train-llm")
    print(f"  → {(time.time()-t1)/60:.1f} dk", flush=True)
    train_bert = np.concatenate([train_bert_orig, train_bert_llm])
else:
    train_bert = train_bert_orig

assert len(train_bert) == len(train_df), \
    f"BERT skor sayısı ({len(train_bert)}) train_df ({len(train_df)}) ile eşleşmiyor!"
print(f"  Toplam train_bert: {len(train_bert):,} "
      f"(orijinal {len(train_bert_orig):,} + LLM {len(train_bert)-len(train_bert_orig):,})", flush=True)
...
if bert_needs_test:
    ...
    np.save(str(v34_test_bert_path), test_bert)
    print(f"  → {v34_test_bert_path.name} | {(time.time()-t1)/60:.1f} dk", flush=True)
else:
    test_bert = np.load(str(v34_test_bert_path))
...
if len(llm_df) > 0:
    print(f"\n  LLM veri kaynak dağılımı: llm_recovered_positive={n_llm_pos:,} "
          f"| llm_verified_negative={n_llm_neg:,}", flush=True)
...
out_path = SUBM / "submission_v35a_llm_verified.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)
np.save(str(SUBM / "v35a_test_proba.npy"), test_proba.astype(np.float32))

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v35a LLM-doğrulamalı hızlı test", flush=True)
print(f"  OOF F1        : {best_f1:.5f} (v34: {V34_OOF})", flush=True)
