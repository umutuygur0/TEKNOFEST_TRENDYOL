...
out_path = SUBM / "submission_v36a_quicktest.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)
np.save(str(SUBM / "v36a_test_proba.npy"), test_proba.astype(np.float32))

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v36a test-dağılımı LLM-doğrulamalı hızlı test", flush=True)
print(f"  OOF F1        : {best_f1:.5f} (v34: {V34_OOF}, v35a: 0.96996)", flush=True)
