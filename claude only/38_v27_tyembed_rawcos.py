"""
38_v27_tyembed_rawcos.py — v27: tyembed_cos RAW (Group Features Kaldırıldı)
============================================================
v26 REGRESYON ANALİZİ:
  - v26 OOF=0.9596 ama Kaggle=0.68 (0.84'ten büyük düşüş)
  - Kök neden: group features (grp_pct, grp_zscore, grp_max, rel_max)
      Train grupları: ~6 item/query (sentetik negatifler)
      Test grupları:  ~100+ item/query (Trendyol retrieval)
      → Farklı boyutlu gruplar → farklı dağılım → model yanlış kural öğrendi
  - #1 feature bert_grp_pct (47K importance) test'te anlamsız oldu

v27 FİXİ:
  - v23 tabanı (35 feature, tam olarak)
  - + tyembed_cos SADECE (ham cosine, group feature YOK)
  - TOPLAM: 36 feature
  - iter=8000, early_stop=200
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE A: VERİ + NEGATİFLER (v23 ile birebir aynı, rng=42)
# ═══════════════════════════════════════════════════════════════════════
t0 = time.time()
print("=" * 65, flush=True)
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE C: TY-ECOMM-EMBED COSINE (ham, grup feature YOK)
# ═══════════════════════════════════════════════════════════════════════
...
print(f"  train_cos: min={train_cos.min():.3f} max={train_cos.max():.3f} mean={train_cos.mean():.3f}", flush=True)
print(f"  tyembed_cos pos mean: {train_cos[train_df['label'].values==1].mean():.3f} "
...
print(f"  test_cos:  min={test_cos.min():.3f} max={test_cos.max():.3f} mean={test_cos.mean():.3f}", flush=True)
print(f"  → TY-ecomm-embed cosine hazır | {(time.time()-t1)/60:.1f} dk", flush=True)

del item_embs, train_q_embs, test_q_embs
gc.collect()

# ═══════════════════════════════════════════════════════════════════════
# PHASE D: FEATURE ENGINEERING (36 feature: 35 v23 + tyembed_cos)
# ═══════════════════════════════════════════════════════════════════════
print("\n[D] FEATURE ENGINEERING (36 feature: 35 v23 + tyembed_cos)...", flush=True)
...
def build_features(df_in, tfidf_vect, bert_arr, tyembed_arr, fit=False, all_texts=None):
    ...
    f = pd.DataFrame()
    # ── v23'ün 35 feature'ı — TAMAMEN AYNI
    ...
    # ── v27 YENİ: sadece ham tyembed cosine (grup feature yok)
...
assert len(FEATS) == 36, f"Beklenmedik feature sayısı: {len(FEATS)}"

X_tr = build_features(
    train_df, tfidf, train_bert, train_cos, fit=True,
...
for feat, imp in fi.head(20).items():
    marker = " ← YENİ" if feat == "tyembed_cos" else ""
...
print(f"  Feature sayısı: {len(FEATS)} (35 v23 + tyembed_cos)", flush=True)
