...
# ─────────────────────────────────────────────────────────────────────
# PHASE B: BERT — bert_v36 MODELİ YENİDEN KULLANILIYOR (yeniden eğitim YOK),
#          sadece YENİ v37 LLM satırları için ek inference
# ─────────────────────────────────────────────────────────────────────
print("\n[B] BERT v36 (cache + sadece YENİ v37 LLM satırları için inference)...", flush=True)
...
# v37a: cache her zaman v36b'nin tabanına (n_original) göre doğrulanır —
# YENİ v37 satırları için ayrıca ve sadece bir kez inference yapılacak.
orig_cache_valid = (v36_train_bert_path.exists() and
                    len(np.load(str(v36_train_bert_path))) == n_original)
...
if orig_cache_valid:
    train_bert_orig = np.load(str(v36_train_bert_path))
    print(f"  Orijinal (v36b) train BERT cache'den: {len(train_bert_orig):,}", flush=True)
else:
    ...
    np.save(str(v36_train_bert_path), train_bert_orig)
    print(f"  → {v36_train_bert_path.name} | {(time.time()-t1)/60:.1f} dk", flush=True)

if len(llm_df) > 0:
    print(f"  YENİ v37 LLM-doğrulamalı satırlar için BERT inference ({len(llm_df):,} çift, "
          f"cache'li bert_v36 modeliyle, yeniden eğitim YOK)...", flush=True)
    t1 = time.time()
...
print(f"  Toplam train_bert: {len(train_bert):,} "
      f"(v36b tabanı {len(train_bert_orig):,} + YENİ v37 {len(train_bert)-len(train_bert_orig):,})", flush=True)
...
out_path = SUBM / "submission_v37a_quicktest.csv"
pd.DataFrame({"id": sub_pairs["id"], "prediction": final}).to_csv(str(out_path), index=False)
np.save(str(SUBM / "v37a_test_proba.npy"), test_proba.astype(np.float32))

print(f"\n{'='*65}", flush=True)
print(f"TAMAMLANDI — v37a hedefli LLM-doğrulamalı hızlı test (+spec_code_match)", flush=True)
print(f"  OOF F1        : {best_f1:.5f} (v36b: {V36B_OOF})", flush=True)
...
print(f"  Feature sayısı: {len(FEATS)} (v36 ile aynı 41 + YENİ spec_code_match)", flush=True)
